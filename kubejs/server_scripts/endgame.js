    ...
    if (isExpertMode) {
        global.basicpolyball.forEach(([id, color]) => {
            ...
            if (Platform.isLoaded(mod)) {
                event.recipes.extendedcrafting.compressor(`kubejs:${material}_polyball`, 'minecraft:diamond', "minecraft:iron_ingot", 512, 5000000, 5000000).id(`kubejs:compressor/${material}_polyball`)
            }
        });
